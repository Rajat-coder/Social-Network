...
def send_fcm_notification(devices, title = '', body = '', image_url = None, data = {}):
    '''
        devices variable can be FCMDevice model object or FCMDeviceQuerySet
        title, body are compulsory parameters
    '''
    fcm_status = False
    if title and (body or image_url):
        ...
        if image_url:
            fcm_request_data['extra_kwargs']['notification']['image'] = image_url
            fcm_request_data['extra_kwargs']['apns'] = {
                'payload': {
                    'aps': {
                        'mutable-content': 1
                    }
        ...
        fcm_res = devices.send_message(**fcm_request_data)
        if fcm_res and fcm_res.get('success', 0) > 0:
            fcm_status = True
    return fcm_status
